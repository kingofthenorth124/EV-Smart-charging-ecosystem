...
import { Bolt, Calendar, ArrowUpRight, ArrowDownRight, RefreshCcw, HandCoins } from "lucide-react";
        ...
        {tab === "TRANSACTIONS" && (
          <div className="animate-in fade-in duration-300">
            <div className="flex gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide">
              {["ALL", "TOPUP", "CHARGE", "REFUND", "ADJUSTMENT"].map((type) => (
                <button
                  key={type}
                  onClick={() => {
            ...
            {isTxLoading ? (
              ...
              <div className="space-y-3">
                {txData.data.map((tx) => {
                  ...
                  return (
                    <div key={tx.id} className="flex items-center gap-3 p-3.5 bg-card border border-border rounded-xl">
                      <div className={`p-2.5 rounded-lg shrink-0 ${
                        tx.type === "TOPUP" ? "bg-[#e6f1fb] text-[#185fa5] dark:bg-blue-950 dark:text-blue-400" :
                        tx.type === "CHARGE" ? "bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-400" :
              ...
              <div className="text-center py-12 bg-card border border-border border-dashed rounded-xl mt-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {txTypeFilter !== "ALL" ? `You have no ${txTypeFilter.toLowerCase()} transactions.` : "Your transaction history is empty."}
                </p>
              </div>
            )}
          </div>
            ...
            {isSessionsLoading ? (
              ...
              <div className="space-y-3">
                {sessionsData.data.map((session) => (
                  <div key={session.id} className="flex items-center gap-3 p-3.5 bg-card border border-border rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground mt-1 flex flex-wrap gap-x-3 gap-y-1">
                        ...
                        {session.elapsedSeconds && session.elapsedSeconds > 0 ? (
                          <span>
                ...
                {sessionsData.total > limit && (
                  <div className="flex justify-between items-center pt-4 pb-2">
                    ...
                    <span className="text-xs text-muted-foreground font-medium">
                      Page {sessionsPage} of {Math.ceil(sessionsData.total / limit)}
                    </span>
                    <Button 
                      variant="outline" 
                      size="sm" 
                      disabled={sessionsPage >= Math.ceil(sessionsData.total / limit)}
                      onClick={() => setSessionsPage(p => p + 1)}
                    >
                      Next
                    </Button>
              ...
              <div className="text-center py-12 bg-card border border-border border-dashed rounded-xl mt-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">You haven't started any charging sessions yet.</p>
              </div>
            )}
          </div>
        )}
