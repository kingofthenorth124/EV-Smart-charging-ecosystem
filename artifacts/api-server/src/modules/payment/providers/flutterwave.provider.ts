...
export class FlutterwaveProvider implements PaymentProvider {
  ...
  async verify(input: VerifyPaymentInput): Promise<VerifyPaymentResult> {
    ...
    const body = await this.request<{
      ...
      `/v3/transactions/${encodeURIComponent(input.providerReference)}/verify`,
      {
        method: "GET",
      },
    );
    ...
    const expectedAmount = input.amountKobo / 100;

    const successful =
      transaction.status === "successful" &&
      transaction.tx_ref === input.reference &&
    ...
    return {
      ...
      amountKobo: Math.round(Number(transaction.amount) * 100),
  ...
  async refund(input: RefundPaymentInput): Promise<RefundPaymentResult> {
    const body = await this.request<{
      ...
      method: "POST",
      body: JSON.stringify({
        id: Number(input.providerReference),
        amount: input.amountKobo / 100,
    ...
    return {
      ...
      amountKobo: Math.round(
        Number(body.data.amount ?? input.amountKobo / 100) * 100,
      ),
      raw: body,
    };
  }
  ...
  verifyWebhook(
    ...
    const signature = headers["flutterwave-signature"] ?? headers["verif-hash"];
