...
function mintExpiredJwt(payload: {
  sub: string;
  email: string;
  role: string;
}): string {
...
describe("Refresh token theft detection", () => {
  ...
  it("an unknown refresh token returns 401", async () => {
    const res = await request(app.getHttpServer())
      ...
      .send({
        refreshToken:
          "totallyunknowntoken00000000000000000000000000000000000000000000",
      });
...
describe("Password reset flow", () => {
  ...
  it("request → capture token → confirm → login with new password works", async () => {
    ...
    const spy = jest
      .spyOn(
        authService as unknown as Record<string, (...args: unknown[]) => void>,
        "notifyPasswordReset",
      )
  ...
  it("password reset revokes existing sessions", async () => {
    ...
    const spy = jest
      ...
      .mockImplementation((_e: unknown, t: unknown) => {
        capturedToken = t as string;
      });
...
describe("Error response contract", () => {
  ...
  it.each(errorCases)(
    "$label has statusCode + message + correlationId",
    async ({ fn }) => {
      const res = await fn();
      expect(res.body).toMatchObject({
...
describe("Concurrent load behavior", () => {
  ...
  it("incrementAndMaybeLock atomically sets lockedUntil in the same operation as the threshold breach", async () => {
    ...
    const result = await identityService.incrementAndMaybeLock(
      dbUser!.id,
      5,
      15,
    );
  ...
  it("valid login is rejected when the atomic increment-lock fires during bcrypt (spy-controlled interleaving)", async () => {
    ...
    const identityService = app.get(IdentityService);
    const originalReset =
      identityService.resetFailedAttempts.bind(identityService);
...
describe("Audit log persistence", () => {
  ...
  it("writes USER_TOKEN_THEFT_DETECTED row when a revoked refresh token is reused", async () => {
    ...
    const rows = await getAuditRows(
      userId,
      AUTH_AUDIT_ACTIONS.TOKEN_THEFT_DETECTED,
    );
...
describe("Rate limiting", () => {
  ...
  it("returns 429 after 10 login requests within the TTL window", async () => {
    ...
    const phone = uniquePhone();
    rlEmails.push(email);
    await request(rlApp.getHttpServer()).post("/api/v1/auth/register").send({
      firstName: "RL",
      lastName: "Test",
      email,
      phone,
