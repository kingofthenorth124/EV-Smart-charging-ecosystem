...
afterAll(async () => {
  await prisma.chargingSession.deleteMany({ where: { stationId: { in: stationIds } } });
...
describe('wallet authorization', () => {
  it('rejects session start for a suspended wallet', async () => {
    ...
    await prisma.wallet.update({ where: { userId }, data: { status: 'SUSPENDED' } });
...
describe('session start races', () => {
  it('parallel starts for one user yield exactly one ACTIVE session', async () => {
    ...
    expect(
      await prisma.chargingSession.count({ where: { userId, status: 'ACTIVE' } }),
    ).toBe(1);
  ...
  it('parallel starts by different users cannot oversubscribe connectors', async () => {
    ...
    const outcomes = await Promise.allSettled(
      users.map((u) => chargingService.startSession(u, { stationId: station.id })),
    );
...
describe('session settlement races', () => {
  it('parallel stops settle exactly once: one CHARGE transaction, one debit, one connector release', async () => {
    ...
    const session = await chargingService.startSession(userId, { stationId: station.id });
    await new Promise((r) => setTimeout(r, 1200)); // accrue ≥1s so cost is non-zero (elapsed is whole seconds)

    const outcomes = await Promise.allSettled(
      Array.from({ length: 5 }, () => chargingService.stopSession(userId, session.id)),
    );
    ...
    const fulfilled = outcomes.filter(
      (o): o is PromiseFulfilledResult<Awaited<ReturnType<ChargingService['stopSession']>>> =>
        o.status === 'fulfilled',
    );
    ...
    const final = await prisma.chargingSession.findUniqueOrThrow({ where: { id: session.id } });
