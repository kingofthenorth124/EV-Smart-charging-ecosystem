 ...
 */
const STATION_SEED = [
  { name: 'Lekki Phase 1 Hub', location: 'Admiralty Way, Lekki', powerKw: 60, connectorType: 'CCS2', connectorsTotal: 4, tariffKoboPerKwh: 25000 },
  { name: 'Victoria Island Plaza', location: 'Adeola Odeku St, VI', powerKw: 120, connectorType: 'CCS2', connectorsTotal: 2, tariffKoboPerKwh: 32000 },
  { name: 'Ikeja City Mall', location: 'Obafemi Awolowo Way, Ikeja', powerKw: 22, connectorType: 'Type 2', connectorsTotal: 6, tariffKoboPerKwh: 18000 },
  { name: 'Yaba Tech Park', location: 'Herbert Macaulay Way, Yaba', powerKw: 50, connectorType: 'CCS2', connectorsTotal: 3, tariffKoboPerKwh: 22000 },
] as const;
...
export class ChargingService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (count === 0) {
      await this.prisma.station.createMany({
        data: STATION_SEED.map((s) => ({ ...s, connectorsAvailable: s.connectorsTotal })),
      });
  ...
  async listStations(): Promise<StationDto[]> {
    const stations = await this.prisma.station.findMany({ orderBy: { name: 'asc' } });
  ...
  private computeLive(
    ...
    balanceKobo: number,
    now = new Date(),
  ): { energyWh: number; costKobo: number; capped: 'LIMIT_REACHED' | 'BALANCE_EXHAUSTED' | null; elapsedSeconds: number } {
    const elapsedSeconds = Math.max(0, Math.floor((now.getTime() - session.startedAt.getTime()) / 1000));
    ...
    if (costKobo >= cap) {
      capped = session.limitKobo !== null && cap === session.limitKobo ? 'LIMIT_REACHED' : 'BALANCE_EXHAUSTED';
      costKobo = cap;
  ...
  private toSessionDto(session: SessionWithStation, live?: { energyWh: number; costKobo: number; elapsedSeconds: number }): SessionDto {
    ...
    return {
      ...
      elapsedSeconds:
        live?.elapsedSeconds ??
        Math.max(0, Math.floor((endReference.getTime() - session.startedAt.getTime()) / 1000)),
  ...
  async startSession(userId: string, dto: StartSessionDto, correlationId?: string): Promise<SessionDto> {
    ...
    if (wallet.balanceKobo < minBalance) {
      await this.auditService.log({
        ...
        result: 'FAILURE',
        correlationId,
        metadata: { reason: 'INSUFFICIENT_BALANCE', balanceKobo: wallet.balanceKobo, minBalanceKobo: minBalance },
      });
    ...
    try {
      session = await this.prisma.$transaction(async (tx) => {
        ...
        if (claimed.count === 0) {
          throw new ConflictException('No connectors are available at this station right now');
        }
        const updated = await tx.station.findUniqueOrThrow({ where: { id: station.id } });
    ...
    return this.toSessionDto(session, { energyWh: 0, costKobo: 0, elapsedSeconds: 0 });
  }

  /** Returns the live active session, auto-finalizing it when a cap is reached. */
  async getActiveSession(userId: string, correlationId?: string): Promise<SessionDto | null> {
    ...
    if (live.capped) {
      return this.finalizeSession(session, live.energyWh, live.costKobo, live.capped, correlationId);
    }
    return this.toSessionDto(session, live);
  }

  async stopSession(userId: string, sessionId: string, correlationId?: string): Promise<SessionDto> {
    const session = await this.prisma.chargingSession.findFirst({
    ...
    const wallet = await this.walletService.getOrCreateWallet(userId);
    const live = this.computeLive(session, wallet.balanceKobo);
    return this.finalizeSession(session, live.energyWh, live.costKobo, live.capped ?? 'USER_STOP', correlationId);
  }
  ...
  private async finalizeSession(
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (station.status === 'BUSY' && station.connectorsAvailable > 0) {
        await tx.station.update({ where: { id: station.id }, data: { status: 'AVAILABLE' } });
      }
  ...
  async listSessions(userId: string, query: PaginationQueryDto): Promise<PaginatedResult<SessionDto>> {
    const where = { userId };
    ...
    return paginate(rows.map((r) => this.toSessionDto(r)), total, query.page, query.limit);
  }
  ...
  async lifetimeStats(userId: string): Promise<{ sessionsCount: number; totalEnergyWh: number; totalSpentKobo: number }> {
