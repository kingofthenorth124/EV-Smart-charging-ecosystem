...
export class ChargingController {
  ...
  listSessions(@CurrentUser('sub') userId: string, @Query() query: PaginationQueryDto) {
  ...
  async getActiveSession(
    ...
    const session = await this.chargingService.getActiveSession(userId, req.correlationId);
    return { session };
  }
  ...
  @ApiResponse({ status: 409, description: 'Session already active or station unavailable' })
  startSession(
    @CurrentUser('sub') userId: string,
