...
describe("passwordResetRequestSchema", () => {
  it("accepts a valid email", () => {
    expect(
      passwordResetRequestSchema.safeParse({ email: "u@e.com" }).success,
    ).toBe(true);
  });
});
...
describe("updateUserStatusSchema", () => {
  ...
  it("rejects an invalid status", () => {
    expect(
      updateUserStatusSchema.safeParse({ status: "DELETED" }).success,
    ).toBe(false);
  });
});
...
describe("startSessionSchema", () => {
  it("accepts a valid session start request", () => {
    expect(
      startSessionSchema.safeParse({ stationId: "station-1" }).success,
    ).toBe(true);
  });

  it("accepts an optional spend cap", () => {
    expect(
      startSessionSchema.safeParse({ stationId: "station-1", limitKobo: 5_000 })
        .success,
    ).toBe(true);
  });
  ...
  it("rejects a spend cap below the minimum", () => {
    expect(
      startSessionSchema.safeParse({ stationId: "station-1", limitKobo: 999 })
        .success,
    ).toBe(false);
  });
});
