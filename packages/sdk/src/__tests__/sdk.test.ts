...
describe('SdkError', () => {
  ...
  it('isForbidden is true for 403', () => {
    expect(new SdkError({ statusCode: 403, message: 'Forbidden' }).isForbidden).toBe(true);
  });

  it('isNotFound is true for 404', () => {
    expect(new SdkError({ statusCode: 404, message: 'Not found' }).isNotFound).toBe(true);
  });

  it('isConflict is true for 409', () => {
    expect(new SdkError({ statusCode: 409, message: 'Conflict' }).isConflict).toBe(true);
  });

  it('isServerError is true for 5xx', () => {
    expect(new SdkError({ statusCode: 500, message: 'Error' }).isServerError).toBe(true);
    expect(new SdkError({ statusCode: 503, message: 'Error' }).isServerError).toBe(true);
  });

  it('isRateLimited is true for 429', () => {
    expect(new SdkError({ statusCode: 429, message: 'Too many requests' }).isRateLimited).toBe(true);
  });
...
describe('isSdkError', () => {
  it('returns true for SdkError instances', () => {
    expect(isSdkError(new SdkError({ statusCode: 400, message: 'Bad' }))).toBe(true);
  });
...
describe('extractErrorMessage', () => {
  ...
  it('returns a fallback for unknown errors', () => {
    ...
    expect(extractErrorMessage(42)).toBe('An unexpected error occurred. Please try again.');
  });
});
...
describe('wrapResult', () => {
  ...
  it('re-throws non-SdkError errors', async () => {
    ...
    await expect(wrapResult(() => Promise.reject(networkErr))).rejects.toThrow('fetch failed');
  });
});
