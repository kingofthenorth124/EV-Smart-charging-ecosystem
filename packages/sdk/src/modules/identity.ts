...
import type { ListUsersParams, UpdateUserStatusRequest } from '@workspace/api-client-react';
...
export const identityModule = {
  async listUsers(params?: ListUsersParams): Promise<PaginatedResult<UserProfile>> {
    try {
      return await apiListUsers(params) as unknown as PaginatedResult<UserProfile>;
    } catch (err) {
      wrapApiError(err);
    }
  },
  ...
  async updateUserStatus(
    ...
    try {
      return await apiUpdateUserStatus(userId, { status }) as unknown as UserProfile;
    } catch (err) {
      wrapApiError(err);
    }
  },
